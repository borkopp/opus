"use client";

import { Card } from "@/components/ui/card";
import { Bot, ChevronDown } from "lucide-react";
import { Button } from "@/components/ui/button";
import Link from "next/link";
import { motion } from "framer-motion";
import { useEffect, useState } from "react";
import type { Variants } from "framer-motion";
import { api } from "@/convex/_generated/api";
import type { FunctionReturnType } from "convex/server";
import { useDashboardI18n } from "@/components/dashboard-i18n-provider";
import { WidgetTitle } from "@/components/dashboard/WidgetTitle";

function StatCounter({
  value,
  locale,
  suffix = "",
  decimals = 0,
}: {
  value: number;
  locale: string;
  suffix?: string;
  decimals?: number;
}) {
  const [displayValue, setDisplayValue] = useState(0);

  useEffect(() => {
    const start = 0;
    const end = value;
    const duration = 1200;
    const startTime = performance.now();

    const update = (currentTime: number) => {
      const elapsed = currentTime - startTime;
      const progress = Math.min(elapsed / duration, 1);
      const ease = 1 - Math.pow(1 - progress, 4); // Ease-out-quart
      const current = start + (end - start) * ease;
      setDisplayValue(current);
      if (progress < 1) requestAnimationFrame(update);
    };

    requestAnimationFrame(update);
  }, [value]);

  return (
    <>
      {displayValue.toLocaleString(locale, {
        minimumFractionDigits: decimals,
        maximumFractionDigits: decimals,
      })}
      {suffix}
    </>
  );
}

interface AIPerformanceProps {
  aiPerformance: FunctionReturnType<typeof api.dashboard.getAIPerformance>;
}

export function AIPerformanceWidget({ aiPerformance }: AIPerformanceProps) {
  const { locale, t } = useDashboardI18n();
  const enabled = aiPerformance?.aiEnabled ?? false;

  const containerVars = {
    hidden: { opacity: 0, y: 15 },
    visible: {
      opacity: 1,
      y: 0,
      transition: {
        duration: 0.6,
        ease: [0.22, 1, 0.36, 1],
        staggerChildren: 0.1,
      },
    },
  } satisfies Variants;

  const itemVars = {
    hidden: { opacity: 0, scale: 0.95, y: 10 },
    visible: {
      opacity: 1,
      scale: 1,
      y: 0,
      transition: { duration: 0.4, ease: [0.22, 1, 0.36, 1] },
    },
  } satisfies Variants;

  return (
    <motion.div
      variants={containerVars}
      initial="hidden"
      animate="visible"
      className="h-full"
    >
      <Card className="flex flex-col h-full p-6 col-span-1 lg:col-span-1 relative overflow-hidden group">
        <div className="flex justify-between items-start mb-6 relative z-10">
          <div className="flex items-center gap-4">
            <motion.div
              whileHover={{ scale: 1.05 }}
              className={`flex items-center justify-center w-12 h-12 rounded-lg ${enabled ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
            >
              <Bot className="h-6 w-6" />
            </motion.div>
            <div>
              <div className="flex items-center gap-2">
                <WidgetTitle>{t("AI Agent", "AI агент")}</WidgetTitle>
                {enabled ? (
                  <motion.div
                    initial={{ scale: 0.8, opacity: 0 }}
                    animate={{ scale: 1, opacity: 1 }}
                    className="flex items-center gap-1 rounded-full border border-border bg-accent px-2 py-0.5"
                  >
                    <div className="w-1 h-1 rounded-full bg-primary animate-pulse" />
                    <span className="micro-label text-accent-foreground">
                      {t("Live", "Во живо")}
                    </span>
                  </motion.div>
                ) : (
                  <div className="px-2 py-0.5 rounded-full bg-muted border border-border flex items-center gap-1">
                    <div className="w-1 h-1 rounded-full bg-muted-foreground" />
                    <span className="micro-label text-muted-foreground">
                      {t("Off", "Исклучено")}
                    </span>
                  </div>
                )}
              </div>
              <p className="text-xs text-muted-foreground mt-1 tracking-wide">
                {t("Autonomous Front-Desk", "Автономна рецепција")}
              </p>
            </div>
          </div>
        </div>

        {enabled ? (
          <>
            <div className="flex flex-col gap-6 flex-1 justify-center relative z-10">
              <motion.div variants={itemVars} className="flex flex-col">
                <div className="flex items-baseline gap-2">
                  <motion.span className="text-5xl font-display font-black text-foreground tracking-tight">
                    <StatCounter
                      value={aiPerformance.totalConversations}
                      locale={locale}
                    />
                  </motion.span>
                  {t("chats", "разговори")}
                </div>
                <span className="micro-label text-muted-foreground mt-2 flex items-center gap-1.5">
                  {t("Monthly Conversations", "Месечни разговори")}
                </span>
              </motion.div>

              <div className="flex flex-col gap-4">
                <motion.div variants={itemVars} className="flex flex-col gap-2">
                  <div className="flex justify-between items-end">
                    <span className="micro-label text-muted-foreground">
                      {t("Autonomous Success", "Автономен успех")}
                    </span>
                    <span className="text-sm font-black text-accent-foreground font-display">
                      <StatCounter
                        value={aiPerformance.bookingRate}
                        locale={locale}
                        suffix="%"
                        decimals={1}
                      />
                    </span>
                  </div>
                  <div className="h-3.5 w-full bg-muted/30 rounded-full border border-foreground/[0.03] p-0.5 shadow-[inset_0_1px_3px_rgba(0,0,0,0.25)] flex items-center overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${aiPerformance.bookingRate}%` }}
                      transition={{
                        duration: 1.5,
                        ease: [0.25, 1, 0.5, 1],
                        delay: 0.5,
                      }}
                      className="h-full bg-primary rounded-full"
                    />
                  </div>
                </motion.div>

                <motion.div variants={itemVars} className="flex flex-col gap-2">
                  <div className="flex justify-between items-end">
                    <span className="micro-label text-muted-foreground">
                      {t("Human Assistance", "Човечка помош")}
                    </span>
                    <span className="text-sm font-black text-muted-foreground font-display">
                      <StatCounter
                        value={aiPerformance.handoffRate}
                        locale={locale}
                        suffix="%"
                        decimals={1}
                      />
                    </span>
                  </div>
                  <div className="h-3.5 w-full bg-muted/30 rounded-full border border-foreground/[0.03] p-0.5 shadow-[inset_0_1px_3px_rgba(0,0,0,0.25)] flex items-center overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${aiPerformance.handoffRate}%` }}
                      transition={{
                        duration: 1.5,
                        ease: [0.25, 1, 0.5, 1],
                        delay: 0.7,
                      }}
                      className="h-full bg-muted-foreground/30 rounded-full drop-shadow-[0_1px_2px_rgba(0,0,0,0.2)]"
                    />
                  </div>
                </motion.div>
              </div>
            </div>

            <motion.div
              variants={itemVars}
              className="relative z-10 border-t border-border pt-5 mt-auto"
            >
              <Button
                variant="ghost"
                className="h-11 w-full justify-between text-xs font-bold text-foreground transition-all duration-300 hover:bg-muted hover:text-foreground group-hover:translate-y-[-2px]"
              >
                {t(
                  "Analysis & Conversation Logs",
                  "Анализа и дневник на разговори",
                )}
                <div className="flex items-center gap-1">
                  <motion.span
                    animate={{ x: [0, 2, 0] }}
                    transition={{ duration: 2, repeat: Infinity }}
                    className="opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                  >
                    {t("View", "Преглед")}
                  </motion.span>
                  <ChevronDown className="h-4 w-4" />
                </div>
              </Button>
            </motion.div>
          </>
        ) : (
          <div className="flex flex-col flex-1 justify-center items-center relative z-10 gap-4 text-center px-2">
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="w-14 h-14 rounded-2xl bg-muted flex items-center justify-center"
            >
              <Bot className="size-7 text-muted-foreground" />
            </motion.div>
            <div>
              <p className="text-sm font-semibold text-foreground">
                {t("AI Agent is off", "AI агентот е исклучен")}
              </p>
              <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                {t(
                  "Enable the AI front-desk to handle bookings and customer messages automatically.",
                  "Овозможете ја AI рецепцијата автоматски да управува со термини и пораки од клиенти.",
                )}
              </p>
            </div>
            <Link
              href="/settings?tab=ai"
              className="text-xs font-medium text-primary hover:underline underline-offset-4 transition-colors"
            >
              {t("Configure in Settings", "Поставки")}
            </Link>
          </div>
        )}
      </Card>
    </motion.div>
  );
}
