...
export function StaffUtilisationWidget({
  staffUtilisation,
}: StaffUtilisationProps) {
  const { locale, t } = useDashboardI18n();
  const totalBooked = staffUtilisation.reduce(
    (sum, s) => sum + s.bookedMins,
    0,
  );
  ...
  return (
    ...
    >
      <Card className="flex flex-col p-5 col-span-1 lg:col-span-1 h-full transition-shadow duration-300 hover:shadow-md">
        ...
        >
          <WidgetTitle>
            {t("Staff Capacity", "Капацитет на персонал")}
          </WidgetTitle>
        </motion.div>

        <motion.div
          variants={itemVars}
          className="flex items-center gap-6 mb-5 pb-5 border-b border-border/20"
        >
          <div className="relative w-20 h-20 flex items-center justify-center shrink-0">
            <svg
              viewBox="0 0 36 36"
              className="w-full h-full transform -rotate-90 drop-shadow-[0_2px_8px_rgba(0,0,0,0.1)]"
            >
              <defs>
        ...
        <div className="flex-1 flex flex-col gap-4 text-sm font-semibold">
          {staffUtilisation.length === 0 ? (
            <span className="text-sm text-muted-foreground text-center py-4">
              {t(
                "No staff capacity data",
                "Нема податоци за капацитет на персонал",
              )}
            </span>
          ) : (
