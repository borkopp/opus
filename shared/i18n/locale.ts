...
export function getClientLocale(): Locale {
  ...
  try {
    ...
    return isLocale(value)
      ? value
      : resolveLocale(navigator.languages.join(","));
  } catch {
...
export function setClientLocale(locale: Locale) {
  ...
  try {
    ...
    const sharedDomain =
      hostname === "opus.mk" || hostname.endsWith(".opus.mk");
    // Remove the old host-only cookie before writing the shared preference.
    if (sharedDomain) {
      document.cookie = `${LOCALE_COOKIE_NAME}=; Path=/; Max-Age=0`;
    }
    document.cookie = `${LOCALE_COOKIE_NAME}=${locale}${sharedDomain ? "; Domain=opus.mk" : ""}; Path=/; Max-Age=31536000; SameSite=Lax${
      isHttps ? "; Secure" : ""
    }`;
  } catch {
