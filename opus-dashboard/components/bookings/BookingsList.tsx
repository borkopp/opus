...
export function BookingsList({
    ...
    const getStatusIndicator = (status: string, isAi: boolean, isSelected: boolean) => {
        if (status === "checked_in" || status === "completed") {
            return <div className="h-2.5 w-2.5 rounded-full bg-success shadow-sm" title="Completed/Checked In" />;
        }
        if (status === "no_show") {
            return <div className="h-2.5 w-2.5 rounded-full bg-danger shadow-sm" title="No Show" />;
        }
        if (status === "cancelled") {
            return <div className="h-2.5 w-2.5 rounded-full bg-danger/50 shadow-sm" title="Cancelled" />;
        }
        if (isAi) {
            return <IconSparkles className="h-3.5 w-3.5 text-primary" title="AI Booked" />;
        }
        return <div className={cn("h-2.5 w-2.5 rounded-full shadow-sm", isSelected ? "bg-primary" : "bg-warning")} title="Upcoming" />;
    };
