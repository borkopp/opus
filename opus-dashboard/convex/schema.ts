...
// and intentionally have no orgId. Every other table MUST be filtered by orgId.
// ─────────────────────────────────────────────────────────────────────────────
...
export default defineSchema({
  ...
  // ───────────────────────────────────────────────────────
  // Platform-scope (no orgId) — these span all orgs because the marketplace
  // searches across every published business. Same exception class as opus_users.
  // ═══════════════════════════════════════════════════════
  ...
  // Append-only history of a marketplace_conversation. Assistant
  // turns also persist the structured `recommendations` returned to
  // the client so the chat can re-render after refresh.
  // ─────────────────────────────────────────────────────
  marketplace_messages: defineTable({
  ...
  // Polymorphic vector store. One row per embeddable chunk
  // (entityType="org" | "service" | "reputation"). The
  // `reputation` rows are AI-summarised review snippets, one
  // per org, regenerated by a daily cron when new reviews land.
  //
  // Filter fields are denormalised because Convex vectorIndex
  // filters are exact-match only (no joins, no range queries).
  // ─────────────────────────────────────────────────────
  marketplace_embeddings: defineTable({
    entityType: v.union(
      v.literal("org"),
      v.literal("service"),
