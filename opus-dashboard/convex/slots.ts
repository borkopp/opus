...
function dayStartAtForDate(date: string): number | null {
...
function datesForCalendarMonth(month: string): string[] | null {
  if (!CALENDAR_MONTH_PATTERN.test(month)) return null;
  const [year, monthNumber] = month.split("-").map(Number);
  if (year < 1_000 || monthNumber < 1 || monthNumber > 12) return null;
...
type QuickBookingResources = {
  settings: Doc<"org_settings">;
  quickBookingDurationMins: number;
  staffMembers: Doc<"staff_members">[];
};

async function loadQuickBookingResources(
  ctx: Pick<QueryCtx, "db">,
  orgId: Id<"orgs">,
): Promise<QuickBookingResources> {
  ...
  const staffWithServices = new Set(
  ...
  return {
    ...
    quickBookingDurationMins:
      ...
      defaultQuickBookingDurationMins(settings.slotDurationMins),
    staffMembers: staffMembers.filter((staff) =>
      staffWithServices.has(staff._id),
    ),
  };
}

async function quickBookingSlotsForDate(
  ctx: Pick<QueryCtx, "db">,
  orgId: Id<"orgs">,
  date: string,
  resources: QuickBookingResources,
) {
  const dayStartAt = dayStartAtForDate(date);
  if (dayStartAt === null) {
  ...
  const slots = (
    await Promise.all(
      resources.staffMembers.map(async (staff) => {
        ...
        return buildQuickBookingSlots({
          freeIntervals: availability.freeIntervals,
          dayStartAt,
          slotDurationMins: resources.settings.slotDurationMins,
          quickBookingDurationMins: resources.quickBookingDurationMins,
          bufferTimeMins: resources.settings.bufferTimeMins,
        }).map((slot) => ({ ...slot, staffId: staff._id }));
      }),
    )
  ).flat();
...
export const getQuickBookingSlots = query({
  args: {
    orgId: v.id("orgs"),
    date: v.string(), // "YYYY-MM-DD"
  },
  handler: async (ctx, args) => {
    await requireAuth(ctx, args.orgId);
    const resources = await loadQuickBookingResources(ctx, args.orgId);
    const slots = await quickBookingSlotsForDate(
      ctx,
      args.orgId,
      args.date,
      resources,
...
export const getQuickBookingAvailableDates = query({
  args: {
    orgId: v.id("orgs"),
  ...
  handler: async (ctx, args) => {
    await requireAuth(ctx, args.orgId);
    const dates = datesForCalendarMonth(args.month);
    if (!dates) throw new ConvexError("Enter a valid booking month.");
...
export type FreeIntervalBoundary =
  | "workingStart"
  | "workingEnd"
  | "booking"
  | "break"
...
export async function computeFreeIntervalsForStaffDate(
  ctx: Pick<QueryCtx, "db">,
  orgId: Id<"orgs">,
  staffId: Id<"staff_members">,
  date: string,
  providedSettings?: Doc<"org_settings">,
) {
  const orgSettings =
    providedSettings ??
    (await ctx.db
  ...
  if (!orgSettings || orgSettings.orgId !== orgId) {
  ...
  let leftSource: FreeIntervalBoundary = nowClipsWindow
    ? "now"
    : "workingStart";
...
export const computeFreeIntervalsForDate = internalQuery({
  ...
  handler: async (ctx, args) =>
    computeFreeIntervalsForStaffDate(ctx, args.orgId, args.staffId, args.date),
});
...
export function classifyFreeIntervals(
  ...
  return freeIntervals.map((interval) => {
    ...
    const bothBounded =
      (interval.leftBoundary === "booking" ||
        interval.leftBoundary === "break") &&
      (interval.rightBoundary === "booking" ||
        interval.rightBoundary === "break");
    ...
    if (nowIsPastWorkingEnd) {
      ...
      classification =
        interval.leftBoundary === "now" ? "now_bounded" : "edge_of_day";
