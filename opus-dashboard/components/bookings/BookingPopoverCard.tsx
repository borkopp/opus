...
export function BookingPopoverCard({
  ...
  const staffDisplayName = staffMember?.displayName || t("Staff", "Член на тим");
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent
        ...
        align={align}
        sideOffset={8}
        className="w-80 sm:w-88 rounded-3xl p-4 sm:p-5 shadow-2xl border border-border/60 bg-card/95 backdrop-blur-md z-50 text-foreground"
      >
            ...
            <div className="rounded-2xl bg-muted/30 dark:bg-muted/10 border border-border/40 p-3 flex flex-col gap-1.5 text-xs">
              ...
              {booking.customer.email && (
                <a
                  ...
                  className="flex items-center gap-1.5 text-muted-foreground hover:text-primary transition-colors text-[11px] truncate"
                >
          ...
          <div className="grid grid-cols-2 gap-2 pt-1 border-t border-border/40">
            {!isCompleted && onComplete && (
              <Button
                variant="outline"
                size="sm"
                className="h-8 text-xs font-medium rounded-xl hover:bg-emerald-500/10 hover:text-emerald-600 hover:border-emerald-500/30"
                onClick={() => {
            ...
            {onRescheduleClick && !isCompleted && !isNoShow && (
              <Button
                variant="outline"
                size="sm"
                className="h-8 text-xs font-medium rounded-xl hover:bg-primary/10 hover:text-primary hover:border-primary/30"
                onClick={() => {
            ...
            {!isCompleted && !isNoShow && onMarkNoShow && (
              <Button
                variant="ghost"
                size="sm"
                className="h-8 text-xs font-medium rounded-xl text-muted-foreground hover:bg-rose-500/10 hover:text-rose-600"
                onClick={() => {
            ...
            {onCancel && (
              <Button
                variant="ghost"
                size="sm"
                className="h-8 text-xs font-medium rounded-xl text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                onClick={() => {
