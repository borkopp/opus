...
export function LatestActivityWidget({ orgId }: { orgId: Id<"orgs"> }) {
  ...
  const handleItemClick = (notificationId: Id<"dashboard_notifications">, bookingId?: Id<"bookings">, isRead?: boolean) => {
    if (!isRead) {
  ...
  return (
    <Card className="min-h-0 gap-0 overflow-hidden md:h-full flex flex-col transition-shadow duration-300 hover:shadow-md">
      ...
      <CardHeader className="shrink-0 pb-4">
        <CardTitle>
          <WidgetTitle>Latest Activity</WidgetTitle>
        </CardTitle>
      </CardHeader>
        ...
        {allNotifications.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full min-h-44 p-6 text-center">
            <Empty className="bg-transparent border-0 p-0">
              <EmptyHeader>
                <EmptyMedia variant="icon" className="h-10 w-10 bg-secondary/80 text-muted-foreground/60 mb-2">
          ...
          >
            <AnimatePresence initial={false}>
              {allNotifications.map((item) => {
                const { icon, iconBg } = getActivityTypeConfig(item.type);
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => handleItemClick(item._id, item.bookingId, item.isRead)}
                  >
                    {/* Icon */}
                    <div
                      className={cn(
