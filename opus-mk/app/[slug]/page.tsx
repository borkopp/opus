...
export default function BusinessProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-28">
      ...
      >
        {/* ── Business Header ── */}
        <motion.div variants={fadeInUp} className="flex items-start gap-4 pt-6 pb-4">
          <motion.div 
          ...
          >
            {profile.logoUrl ? (
              <Image src={profile.logoUrl} alt={profile.name} fill className="object-cover transition-transform duration-500 hover:scale-110" sizes="64px" />
            ) : (
          ...
          <div className="flex-1 min-w-0 pt-1.5">
            ...
            {profile.tagline && (
              <p className="text-sm text-muted-foreground mt-0.5 font-medium">{profile.tagline}</p>
            )}
            <div className="flex items-center gap-4 mt-2">
              {profile.averageRating > 0 && (
                <div className="flex items-center gap-1.5 text-xs">
                  <IconStarFilled size={14} className="text-rating drop-shadow-sm" aria-hidden="true" />
                  <span className="font-bold">{profile.averageRating.toFixed(1)}</span>
                  <span className="text-muted-foreground font-medium">({profile.reviewCount})</span>
                </div>
              )}
        ...
        {/* ── Contact pills ── */}
        <motion.div variants={fadeInUp} className="flex items-center gap-2.5 pb-4 overflow-x-auto scrollbar-none px-1 -mx-1">
          {profile.phone && (
            <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href={`tel:${profile.phone}`} className="shrink-0">
              <Badge variant="secondary" className="rounded-full px-3.5 py-1.5 gap-1.5 hover:bg-secondary/80 transition-colors shadow-sm cursor-pointer">
          ...
          {profile.instagramHandle && (
            <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href={`https://instagram.com/${profile.instagramHandle}`} target="_blank" rel="noopener noreferrer" className="shrink-0">
              <Badge variant="secondary" className="rounded-full px-3.5 py-1.5 gap-1.5 hover:bg-secondary/80 transition-colors shadow-sm cursor-pointer">
          ...
          {profile.websiteUrl && (
            <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href={profile.websiteUrl} target="_blank" rel="noopener noreferrer" className="shrink-0">
              <Badge variant="secondary" className="rounded-full px-3.5 py-1.5 gap-1.5 hover:bg-secondary/80 transition-colors shadow-sm cursor-pointer">
        ...
        {profile.bio && (
          <motion.div variants={fadeInUp}>
            ...
            <div className="py-4">
              <h2 className="text-[13px] font-bold uppercase tracking-wider text-muted-foreground mb-3">About</h2>
              <p className="text-[15px] text-foreground/90 leading-relaxed font-medium">{profile.bio}</p>
            </div>
          </motion.div>
        )}
                              ...
                              <div className="flex items-center gap-3.5 flex-1 min-w-0 mr-3">
                                {service.photoUrl && (
                                  <div className="w-14 h-14 rounded-xl bg-secondary shrink-0 overflow-hidden relative shadow-sm">
                                    <Image src={getImageUrl(service.photoUrl)!} alt={service.name} fill className="object-cover transition-transform duration-500 group-hover:scale-110" sizes="56px" />
                                  </div>
                                )}
                                <div className="min-w-0">
                                  <p className="text-[15px] font-bold text-foreground group-hover:text-primary transition-colors duration-300">{service.name}</p>
                                  ...
                                  <div className="flex items-center gap-2 mt-1.5">
                                    <span className="text-[12px] font-bold text-muted-foreground flex items-center gap-1.5">
                                      <IconClock size={14} className="text-muted-foreground/70" aria-hidden="true" />
        ...
        {profile.coordinates && (
          <motion.div variants={fadeInUp}>
            <Separator className="my-2" />
            <div className="py-4">
              <h2 className="text-[13px] font-bold uppercase tracking-wider text-muted-foreground mb-3">Location</h2>
